...
- 每次历史构建生成一个 `build_operation_uid`，写入该次构建的投影事件和维护任务摘要，不新增数据库列或 schema 版本。
- 单个批次失败后由持久任务按配置自动重试；重启时恢复尚未到期的重试时间，后续事件不能越过正在冷却或已经耗尽重试的最早任务。
- 管理员可取消进行中或失败任务。若任务属于历史构建，取消会传播到同一 `build_operation_uid` 下所有尚未完成的任务和事件；已完成任务及已原子发布的事实 revision 不回滚。
- “从断层继续构建”只重新排队失败或已取消事件。关系阶段失败时保留成功的事实检查点，续建不会再次发布该批事实；全部剩余事实批次完成后再统一重建人格关系。
- 失败任务可人工重试，重试次数从零重新计算并复用原任务与阶段检查点。已完成任务可单独删除，也可在当前作用域批量清空；删除任务记录不删除 Timeline、画像 revision 或投影历史。
...
| `user_profile.maintenance_batch_candidate_limit` | int | `16` | `1-256` | 单批最多交给模型判断的候选事实数；单条 Timeline 超限时仍单独处理 |
| `user_profile.maintenance_prompt_max_chars` | int | `16000` | `4000-200000` | 事实提示目标字符上限；优先保留候选，再裁剪已有事实和历史行为证据 |
| `user_profile.maintenance_request_timeout_seconds` | int | `180` | `30-1800` | 单次模型业务调用的总时间上限 |
| `user_profile.fact_maintenance_context_limit` | int | `200` | `20-2000` | 单次事实维护提供给模型的已有事实数量上限 |
| `user_profile.maintenance_max_retries` | int | `3` | `0-10` | 持久维护任务的自动重试次数；耗尽后保留断层供人工重试 |
...
8. **任务状态**：当前阶段、阶段进度、批次 Timeline/候选数量、提示估算、耗时、错误、自动重试、取消、删除和断层续建。
...
- 历史事实按 Timeline 数、候选事实数和提示字符三重边界分批；全部事实完成后，历史关系无 10,000 条截断地分批读取和维护，最后只发布一次完整结果。
