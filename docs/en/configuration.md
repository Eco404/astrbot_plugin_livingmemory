...
| Models and tasks | Dedicated or fallback LLM provider, concurrency, batching, existing-fact context limit, retries, and backoff |
| Fact admission | Truth confidence, long-term profile value, and pending retention |
| Inference and sensitive data | Independent Timeline count, evidence span, confidence, cross-batch evidence pool, and sensitive-inference switch |
...
Objective facts and relationship state use separate model calls. Provider retries do not consume the per-batch business-call allowance. When a successful response violates the JSON or source-reference contract, two whitelist-guided correction attempts run by default without relaxing validation. One fact namespace is always maintained serially. Fact maintenance supplies at most 200 relevant existing facts by default and excludes long-term archived, excluded, and superseded rows. Relationship behavior controls how state may influence a reply; subjective relationship text never becomes an objective fact.
...
A profile lifecycle scan runs every 24 hours by default. It advances expired active facts to stale, archives due pending and long-stale facts, and removes completed jobs, superseded projection revisions, and unreferenced inactive sources beyond their retention periods. Full relationship rebuild reads all projection history and sends meaningful Timeline rows to the model in batches of 32 by default.
