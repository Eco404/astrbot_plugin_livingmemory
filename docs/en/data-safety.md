...
Version 3.8.x uses schema v10.5. The public migration path is:
...
The v10.3 to v10.4 migration only creates user-profile, provenance, conflict, relationship-revision, account-binding, sharing, and maintenance-job tables. The v10.4 to v10.5 migration only creates the derived legacy-Timeline identity-review sidecar. Migration does not scan or backfill old Timeline and does not modify Timeline, Topic, conversations, or supplemental profiles. New private Timeline changes are processed after upgrade; old history requires an explicit administrator scan or rebuild.
