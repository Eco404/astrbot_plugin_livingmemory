...
“重置画像”清除当前派生结果但保持启用，之后只处理新变化；“删除并停用”同时删除画像与关系状态并阻止自动重新建档。两者都不同于历史重建。历史重建会先显示精确可归属 Timeline、身份歧义、缺失记录、当前事实和人工覆盖数量，并同时使用画像与历史指纹拒绝过期提交。身份不明确的旧 Timeline 不会被猜测归属；可归属历史先按有界批次完成客观事实，在末批集中进行行为候选发现与严格发布判定，最后才使用当前同 ID persona 统一重建关系。行为阶段使用短来源引用、可读本地时间和有界的语义/时间候选补证；补证只提高召回，Timeline 数、跨度、置信度、画像价值和敏感信息规则仍由应用层复算。前一阶段失败不会越过检查点继续执行；任务页会显示当前批次 Timeline、候选事实、提示估算、事实/行为/关系阶段耗时和自动重试状态。
...
维护任务支持按状态操作：进行中或失败任务可以取消，失败任务也可以重试，已完成任务可以删除或批量清空。历史构建的批次共享同一构建标识；取消其中一个批次会取消该次构建尚未完成的批次和事件，但已经原子发布的事实 revision 不回滚。取消或自动重试耗尽后，可使用“从断层继续构建”只恢复未完成事件，最后再统一重建人格关系。
