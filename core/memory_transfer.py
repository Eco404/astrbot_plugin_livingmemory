"""Portable Timeline memory import/export helpers."""
...
# These values belong to one physical database/index generation. Importing them
# would either collide with local identities or leave stale derived state.
...
def portable_metadata(metadata: dict[str, Any] | None) -> dict[str, Any]:
    """Remove physical identity and derived-index fields from metadata."""
    cleaned = {
        str(key): value
        for key, value in dict(metadata or {}).items()
        if str(key) not in _INTERNAL_METADATA_KEYS
    }
    source_window = cleaned.pop("source_window", None)
    if isinstance(source_window, dict) and source_window:
        cleaned["imported_source_window"] = source_window
    return cleaned
...
def serialize_transfer_csv(records: list[dict[str, Any]]) -> str:
    """Serialize records to CSV with JSON-encoded structured fields."""
    ...
    for record in records:
        metadata = portable_metadata(record.get("metadata"))
        writer.writerow(
            {
                "original_id": record.get("original_id", ""),
                "content": _escape_csv_formula(record.get("content", "")),
                "importance": record.get("importance", 0.5),
                "session_id": _escape_csv_formula(record.get("session_id") or ""),
                "persona_id": _escape_csv_formula(record.get("persona_id") or ""),
    ...
    if not any(key in payload for key in (*_CONTENT_KEYS, *_SOURCE_KEYS)) and payload:
        ...
        for original_id, item in payload.items():
            if isinstance(item, dict):
                ...
                mapped_entries.append({"original_id": original_id, "content": item})
            else:
                mapped_entries = []
                break
        if mapped_entries:
...
def _as_conversation_entries(value: Any) -> list[Any]:
    ...
    return [{"messages": item} if isinstance(item, list) else item for item in value]
...
def _unescape_csv_row(row: dict[str, Any]) -> dict[str, Any]:
    scalar_fields = {"content", "session_id", "persona_id", "memory_type"}
    return {
