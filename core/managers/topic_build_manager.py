...
15. Preserve that first-person memory voice when it is useful. Use exact human names
   ...
   `示例甲请唯估算，唯回复了结果` is wrong and must become
   `示例甲请我估算，我回复了结果`; ordinary text such as `唯一方案`
...
11. In the summary and atoms, refer to every mapped assistant persona as 我/我的,
    ...
    targets. For an assistant named `唯`, rewrite `示例甲请唯估算，唯回复了结果`
    as `示例甲请我估算，我回复了结果`, but preserve `唯一方案` and the
