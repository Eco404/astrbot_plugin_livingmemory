        ...
        try:
            ...
            for item in task_data["items"]:
                ...
                if item["status"] not in {"queued", "failed"}:
                    continue
                try:
                    result = await self._rebuild_item(task_uid, item)
                    rebuilt_by_space[item["memory_space_id"]].append(item["memory_uid"])
            ...
            if not cancel_requested_before_sync:
                await self._set_task(
                    task_uid, status="running", step="topic_sync"
                )
            ...
            final_status = "cancelled" if cancel_requested else (
                "completed_with_errors" if counts["failed_count"] else "completed"
            )
            await self._set_task(
                task_uid,
                status=final_status,
