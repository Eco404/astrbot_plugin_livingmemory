...
TIMELINE_SETTING_DEFINITIONS: dict[str, dict[str, Any]] = {
    ...
    "recall_engine.recent_context_max_age_seconds": {"default": 7200, "type": "int", "min": 0, "max": 604800, "category": "recall", "label": "扩展上下文最大间隔（秒）", "description": "只使用该时间范围内的历史消息扩展召回，避免旧话题干扰；0 表示不限制时间。", "visible_when": {"key": "recall_engine.inject_with_recent_context", "equals": True}},
    "recall_engine.assistant_context_mode": {"default": "exclude", "type": "select", "options": ["exclude", "low_weight", "normal"], "category": "recall", "label": "跨轮扩展中的 Bot 回复", "option_labels": {"exclude": "不查询", "low_weight": "低权重", "normal": "正常查询"}, "description": "控制 Bot 回复是否参与跨轮次扩展，仅在开启跨轮扩展时生效。", "visible_when": {"key": "recall_engine.inject_with_recent_context", "equals": True}},
    "recall_engine.recent_user_weight": {"default": 0.45, "type": "float", "min": 0.0, "max": 1.0, "step": 0.05, "category": "recall", "label": "recent_user 查询权重", "description": "最近历史用户消息查询分支的权重；当前用户消息始终为 1.0。", "visible_when": {"key": "recall_engine.inject_with_recent_context", "equals": True}},
    "recall_engine.recent_assistant_weight": {"default": 0.40, "type": "float", "min": 0.0, "max": 1.0, "step": 0.05, "category": "recall", "label": "recent_assistant 查询权重", "description": "Bot 历史回复在“正常查询”下的权重；“低权重”模式使用该值的一半。", "visible_when": {"all": [{"key": "recall_engine.inject_with_recent_context", "equals": True}, {"key": "recall_engine.assistant_context_mode", "not_equals": "exclude"}]}},
    ...
    "reflection_engine.summary_trigger_rounds": {"default": 10, "type": "int", "min": 1, "max": 100, "category": "generation", "label": "基础总结轮次", "description": "累计达到该对话轮次后检查是否生成 Timeline；开启话题接续时可延后到自然话题边界。"},
    "reflection_engine.topic_continuation_enabled": {"default": True, "type": "bool", "category": "generation", "label": "话题接续", "description": "达到基础轮次后，若最新对话仍在延续窗口内已有话题则暂缓总结；新话题出现时封存此前内容。"},
    "reflection_engine.topic_continuation_force_summary_rounds": {"default": 20, "type": "int", "min": 2, "max": 200, "category": "generation", "label": "话题接续强制总结上限", "description": "即使话题持续延续，达到该对话轮次后也会强制总结，限制上下文与模型调用成本。", "visible_when": {"key": "reflection_engine.topic_continuation_enabled", "equals": True}},
    ...
    "reflection_engine.idle_summary_delay_minutes": {"default": 30.0, "type": "float", "min": 1.0, "max": 10080.0, "step": 1.0, "category": "generation", "label": "空闲触发时间（分钟）", "description": "会话最后一条消息经过该时间后进入空闲总结检查。", "visible_when": {"key": "reflection_engine.idle_summary_enabled", "equals": True}},
    "reflection_engine.idle_summary_min_rounds": {"default": 3, "type": "int", "min": 1, "max": 100, "category": "generation", "label": "空闲总结最少轮次", "description": "只有未总结对话达到该轮数时才执行空闲总结，避免插件刚启用时产生过碎的 Timeline。", "visible_when": {"key": "reflection_engine.idle_summary_enabled", "equals": True}},
    "reflection_engine.idle_summary_scan_interval_seconds": {"default": 60, "type": "int", "min": 30, "max": 3600, "category": "performance", "label": "空闲会话扫描间隔（秒）", "description": "后台检查空闲会话的时间间隔；只执行本地数据库查询，命中后才调用 LLM。", "visible_when": {"key": "reflection_engine.idle_summary_enabled", "equals": True}},
    ...
    "graph_memory.second_hop_weight": {"default": 0.4, "type": "float", "min": 0.0, "max": 1.0, "step": 0.05, "category": "graph", "label": "二跳候选权重", "description": "仅在图扩展为二跳时控制间接候选的权重。", "visible_when": {"key": "graph_memory.expansion_hops", "equals": 2}},
    ...
    "cloudflare_rerank.retry_base_delay": {"default": 1.0, "type": "float", "min": 0.0, "max": 60.0, "step": 0.5, "category": "model", "label": "Cloudflare 重试等待（秒）", "description": "插件内置 Rerank 客户端重试的基础退避时间。", "visible_when": {"key": "cloudflare_rerank.max_retries", "greater_than": 0}},
    ...
    "forgetting_agent.cleanup_days_threshold": {"default": 30, "type": "int", "min": 1, "max": 3650, "category": "lifecycle", "label": "自动清理天数阈值", "description": "超过该天数的 Timeline 才会进入低重要性清理判断。", "visible_when": {"key": "forgetting_agent.auto_cleanup_enabled", "equals": True}},
    "forgetting_agent.cleanup_importance_threshold": {"default": 0.3, "type": "float", "min": 0.0, "max": 1.0, "step": 0.01, "category": "lifecycle", "label": "自动清理重要性阈值", "description": "重要性低于该值且超过时间阈值的 Timeline 可被自动清理。", "visible_when": {"key": "forgetting_agent.auto_cleanup_enabled", "equals": True}},
...
def validate_timeline_settings(values: dict[str, Any]) -> None:
    """Validate relationships that cannot be checked one setting at a time."""
    if not bool(values.get("reflection_engine.topic_continuation_enabled", True)):
        return
    base_rounds = int(values.get("reflection_engine.summary_trigger_rounds", 10))
    force_rounds = int(
        values.get("reflection_engine.topic_continuation_force_summary_rounds", 20)
    )
    if force_rounds <= base_rounds:
        raise ValueError("话题接续强制总结上限必须大于基础总结轮次")
