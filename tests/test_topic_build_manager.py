...
def test_build_run_context_captures_an_immutable_supplemental_profile_snapshot():
    ...
    assert context.supplemental_identity_profiles[0].display_name == "空雨"
...
def test_private_unbound_timeline_inherits_matching_role_binding_from_peer_timeline():
    ...
    bound = TimelineTopicCandidate(
        ...
        role_bindings={
            "actors": [
                {
                    "actor_id": "aiocqhttp:human:1141337347",
                    "actor_type": "human",
                    "platform": "aiocqhttp",
    ...
    assert actor["actor_id"] == "qq:human:1141337347"
...
def test_private_unbound_same_name_is_session_scoped_across_fragments():
    ...
    first_actor = _pending_actor("空雨", "subject")
    second_actor = _pending_actor("空雨", "requester")
...
def test_private_profile_name_resolves_legacy_actor_to_session_peer():
    ...
    assert roles["human_participants"][0]["observed_names"] == ["空雨", "小雨"]
    assert manager._candidate_identity_payload([candidate])[0]["display_name"] == "空雨"
...
def test_topic_prompts_include_only_stable_id_matched_supplemental_profiles():
    ...
    candidate = TimelineTopicCandidate(
        ...
        summary="空雨说他会完成测试",
        key_facts=["空雨是测试负责人"],
        atom_contents=["空雨会完成测试"],
        atom_fingerprints=["c" * 64],
        role_bindings={
            "actors": [
                {
                    "actor_id": "qq:human:1141337347",
                    "actor_type": "human",
                    "platform": "qq",
                    "sender_id": "1141337347",
                    "observed_names": ["空雨"],
    ...
    fragment = _topic_fragment(1)
    fragment.summary = "空雨说他会完成测试"
    fragment.facts[0]["content"] = "空雨是测试负责人"
    fragment.metadata["participant_refs"] = [
        {
            "actor_id": "qq:human:1141337347",
            "actor_type": "human",
...
def test_fragment_source_attribution_rebinds_timeline_local_actor_refs():
    ...
    assert actor_refs["A1"]["actor_id"] == "qq:human:1141337347"
    assert actor_refs["A2"]["actor_id"] == "assistant-persona:唯"
    ...
    synthesis_payload, _, _ = manager._synthesis_llm_context([fragment])
    assert "assistant-persona:唯" in json.dumps(
...
def test_generic_human_role_repair_requires_one_unambiguous_human():
    ...
    assert repaired == "我提醒空雨检查用户设置，并等待空雨回复"
...
def test_role_payload_enriches_exact_actor_with_supplemental_profile():
    ...
    assert human["actor_id"] == "qq:human:1141337347"
    assert human["resolution_sources"] == ["timeline_role_bindings"]
    assert human["identity_confidence"] == 0.95
    assert human["supplemental_identity_hint"]["display_name"] == "空雨"
