...
async def test_store_projects_topic_from_live_timeline_without_drift(tmp_path: Path):
    ...
    assert first.importance == second.importance == pytest.approx(0.44)
    ...
    assert restored.base_importance == pytest.approx(0.88)
    assert restored.importance == pytest.approx(0.88)
