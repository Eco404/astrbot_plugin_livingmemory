...
| **有界增量维护**<br>新增 Timeline 只与有限数量的既有 Topic 近邻匹配。 | **Agent 原生工具**<br>`recall_long_term_memory` 与 `memorize_long_term_memory` 支持主动使用记忆。 | **明确的数据生命周期**<br>备份、暂存编辑、导入导出、清理、归档与同 ID 重构都有独立操作。 |
