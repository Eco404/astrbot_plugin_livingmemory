...
- **Topic 记忆**: 在原 Timeline 层之上构建可溯源、只读的主题记忆，支持全量/增量维护与可选 Rerank；召回以 Topic 为主体，并使用第三人称正式片段轻量补充，旧库未重构时兼容回退 Timeline。
...
- 相关话题图是无方向、无父子层级的稀疏连接。它会忽略纯日期、时刻和拆分后的年月日结构词；`BW2026` 等含字母的命名标识仍会保留，避免共同发生日期被误当成话题关系证据。
...
- 数据库 v9.6 将 Topic 片段及其 revision 关系正式化，并回填旧 Topic 的片段来源关系；迁移不调用模型，也不生成或改写 Topic。旧片段需在后续重构中完成 Bot 第一人称到第三人称角色映射后才参与片段补充；是否参与召回由 `topic_memory.enabled` 与 `topic_memory.recall_enabled` 控制。
