...
- **正式 Topic 片段与第三人称补充（数据库 v9.6）**：构建期片段从临时中间产物升级为按 Topic revision 关联的正式可追溯对象；片段保留独立 Embedding、事实、时间范围和 Timeline 来源。Timeline 原文明确按 Bot 第一人称解释，并依据 `persona_id` 与 WebUI 人物资料映射，将片段标题、摘要和事实规范为第三人称；生成后会拒绝未转换的第一人称或把已知人物降级为“用户/对方”的输出并校正一次。正式召回改为 Topic 主体加相关片段，只有旧库尚无安全正式片段时才兼容回退整条 Timeline。
...
- **受影响局部 Topic 重构（数据库 v9.5）**：新增 Timeline 不再仅凭单次 Embedding 分数贪心追加到一个既有 Topic。增量任务会以新 Timeline 为种子，补入与全量构建一致的稳定时间簇、语义邻域和聚焦既有 Topic 的完整来源，然后复用全量构建的候选分组、片段提取、Embedding、双向 Rerank、组件约束与合成管线；局部范围过大时退化为只创建安全新 Topic，不截断旧 Topic 来源。
...
### 兼容性
- Topic 功能默认关闭，v9→v9.8 迁移只创建结构和回填已有片段关系，不调用模型、不生成 Topic，也不改变现有 Timeline/图谱。旧 Topic 的 v9.8 人物关系表保持为空，并在下一次全量或局部重构时回填；v9.5 旧片段因尚未经过角色规范，不会直接作为片段补充。
