...
- **Topic 关系误连与稠密节点挤占**：共享宽 Timeline 降为弱证据，不能再单独促成关系；关系算法 v6 用统一的“覆盖收益 + 关系质量”目标替代多轮顺序敏感贪心，并以不降低覆盖为前提全局选择孤立点换边。中文关键词的重叠 n-gram 只计一次，人物代词、身份、边界等抽象连接词不再充当独立证据；稀有精确概念和 `BW2026` 等标识仍可保留低分但明确的关系。参与者索引统一平台别名，并以人格锚定 Bot 身份，避免同一用户或 Bot 因适配器名和回退来源重复出现。
