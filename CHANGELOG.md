...
- 默认关系预留由 200 调整为 300 字符；有日期计划宽限为 3 天，无日期计划 30 天复核，并修正当前 persona 历史关系重建、过期事实状态和人工固定的时效语义。

### 兼容性

- 数据库升级到 v10.5。v10.3 -> v10.4 创建画像表，v10.4 -> v10.5 创建派生身份审查旁表；迁移本身不扫描或回填旧 Timeline、不调用模型、不改写原记录，也不自动合并跨平台账号。
