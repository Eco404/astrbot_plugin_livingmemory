...
- 用户画像维护任务新增事实、跨时间行为和人格关系三阶段检查点与进度；失败重试不会重复已完成阶段，任务可取消、删除、批量清理并从断层继续。
- 跨时间行为改为候选发现与严格发布判定两阶段，使用短来源引用、可读本地时间及有界语义/时间补证，减少规律行为漏提取和选择偏差；发布仍执行独立 Timeline、跨度、置信度和画像价值硬校验。
- 已发布行为的新证据会合并到原事实并更新证据统计，禁止静默改写结论或创建已关联事实的近义重复；发布契约错误可在有限次数内纠正。
- 旧摘要按确定性完全重复或明确包含关系保守分组，减少历史构建请求量，同时继续保持弱证据和 pending 边界。

## [3.8.1] - 2026-08-12

### 新增

- WebUI 侧边栏新增只读用户画像页面，用于查看客观事实、待复核事实、实际注入预览和 persona 关系状态。
- 用户画像维护页支持从身份与 Bot/persona 归属明确的私聊 Timeline 构建尚未生成的画像。

### 优化

- 加宽用户画像维护页的状态筛选栏，并将设置页的用户画像栏目移动到 Topic 右侧。

## [3.8.0] - 2026-08-11

### 新增

- 为具有稳定账号 ID 的私聊用户自动维护可溯源客观画像，记录明确自述和满足证据门槛的偏好、习惯、状态、计划与交流偏好；冲突、时效和敏感信息均有独立治理规则。
- 新增 persona 隔离的人格关系状态，以六个连续维度、主观叙述和短期余韵保持 Bot 对当前用户的态度连续性，并支持冻结、人工调整、重置与 revision 回滚。
- 私聊请求可在独立字符预算内注入当前用户画像；主动召回工具仅在显式设置 `include_user_profile=true` 时返回当前用户画像。
- 维护中心新增用户画像栏目，支持事实和冲突治理、历史重建、账号人工绑定/解绑、客观画像显式共享及维护任务重试。
- 旧版私聊 Timeline 可利用同 session 稳定账号与会话消息证据自动确定归属；无法唯一确定的记录进入可审查队列，支持人工绑定、可逆忽略和恢复考虑。

### 优化

- 历史重建会从现有 Timeline 中按私聊账号、Bot 和 persona 精确发现可归属记录，预览身份歧义与缺口，并以历史指纹保证回填幂等且拒绝过期提交。
- 缺少消息级事实归属的旧摘要只作为弱候选：客观事实保持 pending，不能静默强化或覆盖；主观关系可保留宽泛连续性，但采用独立初始上限和更新限幅。
- 客观画像将事实真实性与长期画像价值分开评估，一次性事件和中低耐久偏好只保留为行为证据；跨批行为推断继续执行独立 Timeline、时间跨度和置信度硬门槛。
- 人格关系改为执行时读取当前 persona，不再在 Timeline 或维护任务中保存人格正文；发布前校验人格签名，支持当前人格协调和完整历史分批重建。
- 注入总上限保持 800 字符，关系优先预算调整为 350 字符且事实与关系预算双向回流；事实维护上下文、白名单契约纠错、生命周期转换、任务保留、投影压缩和历史分页均增加可配置边界。

### 兼容性

- 数据库升级到 v10.4。全部用户画像、人格关系和派生身份审查表由唯一的 v10.3 -> v10.4 迁移创建；迁移本身不扫描或回填旧 Timeline、不调用模型、不改写原记录，也不自动合并跨平台账号。

## [3.7.0] - 2026-08-10

### 变更

- Timeline 总结失败后保留精确窗口并按顺序补偿，重试上限不再造成永久阻塞；出现新的总结边界或模型配置变化时可立即恢复。
- 增量 Topic 构建改为整批联合匹配，以受限的事件连续性和时间邻近度辅助模糊片段，减少连续事件事实误并到较早相似 Topic。
...
- 补充批量操作审计、部分失败反馈、移动端适配及相关回归测试。
- 修正文档中的 Timeline 检索/注入正文说明，补充 FAISS 版本建议和原项目署名。
...
- WebUI 新增手机与横屏窄视口适配，优化导航、表格、维护面板和弹窗布局。
- 修复已归档 Topic 批量删除确认后未提交请求，以及不活跃 Timeline 恢复的同类问题。

## [3.3.0] - 2026-08-01

- Topic 列表新增关键字与 Embedding 相关性搜索，并保留状态和人物筛选。
- 修复 Topic 拆分/合并预览后选择复位，以及维护、召回和保存操作重复提交的问题。
- 完善 Topic 审查任务恢复、过期预览处理和待同步状态展示。

## [3.2.0] - 2026-07-30

- 重构中英文 VitePress 文档站，补全 Timeline、Topic、召回、维护与数据安全说明。
- 更新三语 README、当前仓库链接及配套架构和流程图。

## [3.1.0] - 2026-07-30

- 新增可迁移的 Timeline 来源快照及 JSON/CSV 导入导出与批量删除。
- 优化构建中间数据治理、数据库维护和隐私检查。
- 完善补充人物资料说明并移除可能误导用户的预填示例。

## [3.0.1] - 2026-07-29

- 新增 Topic 构建中间数据治理、数据库压缩及可配置的自动清理。
- 新增 Timeline JSON/CSV 导入导出、来源快照迁移和批量删除。
- 修复 SQLite 压缩语句未关闭及存储维护进度状态残留问题。
