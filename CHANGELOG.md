...
- **Topic 审查与人物关系治理（数据库 v9.13）**：维护页新增待审查队列，集中展示歧义片段、候选 Topic、匹配依据、人物和 Timeline 来源，并支持合并到指定 Topic、新建、暂缓或忽略。新增带变更预览和二次确认的 Topic 合并/按正式片段拆分；主 Topic 或主分组保留原 UID，正文、原子、片段、人物索引和相关话题在同一原子发布中更新，失败时完整回滚。Topic 列表支持按人物筛选，详情中的人物关联事实与诊断来源默认折叠。删除权威人物资料前会展示受影响 Topic/Timeline，可选择排队或立即同步重建；已有 Timeline 正文不会被删除或改写。
...
- **事实级 Topic 人物关系（数据库 v9.8）**：正式片段以受约束的局部 actor ref 记录实际参与者、被提及人物及 speaker/narrator/responder/subject/mentioned/executor/requester 语义角色；无法绑定稳定账号的人物使用片段局部 unresolved ID，禁止按昵称跨片段合并。LLM 可主动请求一次原始消息补证，Topic 与事实人物关系会连同原子、片段和 Timeline 来源在同一事务中写入独立索引；Topic 详情展示身份状态、角色、置信度和事实来源。Topic 召回命中当前人物时只做可配置的轻量加分，不作为硬过滤。人物资料变化会标记受影响 Topic 并复用自动维护队列局部重建，人物资料页也可立即唤醒待同步任务。
